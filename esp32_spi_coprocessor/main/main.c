...
static esp_err_t submit_frame(report_frame_t *frame, uint32_t ref_seq)
{
    ...
    report_mode_t frame_mode;

    lock_status();
    reporting_enabled = s_status.reporting_enabled;
    unlock_status();

    if (!reporting_enabled) {
        frame_mode = (frame->mode == REPORT_MODE_FULL) ? REPORT_MODE_FULL : REPORT_MODE_SUMMARY;
        err = cloud_client_set_reporting(true, frame_mode);
        if (err != ESP_OK) {
        ...
        s_status.report_mode = frame_mode;
        copy_string(s_status.last_error, sizeof(s_status.last_error), "report_auto_enabled");
        unlock_status();
        (void) persist_runtime_reporting(true, frame_mode);
        ESP_LOGW(TAG, "Auto-enabled reporting from incoming %s frame_id=%" PRIu32,
                 (frame_mode == REPORT_MODE_FULL) ? "full" : "summary",
                 frame->frame_id);
    }
...
static void process_cloud_event(const cloud_client_event_t *event)
{
    ...
    case CLOUD_CLIENT_EVENT_REPORT_RESULT:
        ...
        if (event->error != ESP_OK || event->http_status < 200 || event->http_status >= 300) {
            s_status.cloud_connected = false;
            if (event->error != ESP_OK || event->http_status == 401 || event->http_status == 404) {
                s_status.registered_with_cloud = false;
            }
        } else {
            s_status.cloud_connected = true;
