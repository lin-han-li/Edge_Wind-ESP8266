...
/* ================= 调试输出配置 =================
 * 控制日志打印的行为。
 * ESP32 连接在 USART2，因此调试日志(printf)不能往 USART2 打，否则会干扰 AT 指令。
...
#define FFT_POINTS (WAVEFORM_POINTS / 2)
    ...
    /* ================= 函数声明 ================= */
    void ESP_Init(void);                // 初始化 ESP32 (AT指令序列)
     ...
     * 目标：ESP32 不再开机自启动连接，由 UI 的“WiFi/TCP/注册/上报/一键连接”按钮触发
     */
    typedef enum
    {
        ESP_UI_CMD_WIFI = 0,
    ...
    void ESP_UI_TaskInit(void);                 // 在 ESP communication task 里调用一次（创建消息队列）
    bool ESP_UI_SendCmd(esp_ui_cmd_t cmd);      // UI 线程调用（不阻塞）
    void ESP_UI_TaskPoll(void);                 // 在 ESP communication task 循环里调用（处理UI命令）
