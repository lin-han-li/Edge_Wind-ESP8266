...
static esp_err_t post_register_request(const app_config_snapshot_t *snapshot)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            ...
            s_registered = false;
            post_event(CLOUD_CLIENT_EVENT_REGISTER_RESULT, err, 0, 0, 0, NULL, "register_read_fail");
        }
    } else {
        s_registered = false;
        post_event(CLOUD_CLIENT_EVENT_REGISTER_RESULT, err, 0, 0, 0, NULL, "register_transport_fail");
    }
                                     ...
                                     bool reporting_enabled)
{
    ...
    if (err != ESP_OK) {
        esp_http_client_cleanup(client);
        post_event(CLOUD_CLIENT_EVENT_REPORT_RESULT, err, 0, frame->ref_seq, frame->frame_id, NULL, "http_open_failed");
        return err;
    }
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            ...
            post_event(CLOUD_CLIENT_EVENT_REPORT_RESULT, err, 0, frame->ref_seq, frame->frame_id, NULL, "report_read_fail");
        }
        if (http_status == 401 || http_status == 404) {
            s_registered = false;
        }
    } else {
        post_event(CLOUD_CLIENT_EVENT_REPORT_RESULT, err, 0, frame->ref_seq, frame->frame_id, NULL, "report_transport_fail");
        s_registered = false;
    }
                                              ...
                                              const char *fault_code)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            ...
            post_event(CLOUD_CLIENT_EVENT_ERROR, err, 0, 0, 0, NULL, "heartbeat_read_fail");
        }
    } else {
        s_registered = false;
        post_event(CLOUD_CLIENT_EVENT_ERROR, err, 0, 0, 0, NULL, "heartbeat_transport_fail");
    }
...
esp_err_t cloud_client_submit_frame(report_frame_t *frame)
{
    ...
    if (queue_cloud_msg(&msg, pdMS_TO_TICKS(1000)) != ESP_OK) {
...
size_t cloud_client_drop_pending_summary_frames(void)
{
    ...
    if (dropped > 0U) {
        ESP_LOGI(TAG, "Dropped %u queued summary frames before full upload", (unsigned) dropped);
    }
    return dropped;
}
