...
#define ESP32_SPI_MIN_XFER_SIZE 512U
...
#define ESP32_SPI_REPORT_RETRY_LIMIT 3U
                                          ...
                                          esp32_spi_packet_t *rx_packet)
{
    ...
    xfer_size = spi_xfer_size_for_payload(payload_len);
    ...
    st = HAL_SPI_TransmitReceive(&hspi2, s_tx_buf, s_rx_buf, xfer_size, ESP32_SPI_XFER_TIMEOUT_MS);
    cs_set(GPIO_PIN_SET);
                                    ...
                                    const char *label)
{
    ...
    for (attempt = 1U; attempt <= ESP32_SPI_REPORT_RETRY_LIMIT; attempt++) {
        s_last_tx_accepted_ref_seq = 0U;
        s_last_tx_result_ref_seq = 0U;
        s_last_tx_result_code = ESP32_SPI_RESULT_PENDING;
        s_last_tx_result_http_status = 0;
        ...
        if (attempt < ESP32_SPI_REPORT_RETRY_LIMIT) {
            printf("[ESP32SPI] full %s retry frame=%lu attempt=%u/%u\r\n",
                   label ? label : "packet",
                   (unsigned long)frame_id,
                   (unsigned int)(attempt + 1U),
                   (unsigned int)ESP32_SPI_REPORT_RETRY_LIMIT);
            HAL_Delay(10U);
        }
    }
    return false;
