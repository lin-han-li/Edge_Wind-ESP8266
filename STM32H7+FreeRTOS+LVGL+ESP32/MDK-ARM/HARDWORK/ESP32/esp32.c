 ...
 * @brief   ESP32 WiFi 模组驱动程序 (STM32H7 专用优化版)
 * @note    核心特性：
...
#define ESP32_SPI_STRESS_SENDLIMIT_MS 50U
#endif
...
static UART_HandleTypeDef *ESP_GetLogUart(void)
{
    ...
    return &huart2; // ⚠️一般不建议：USART2 用于 ESP32 通信，混用会干扰协议
#else
    extern UART_HandleTypeDef huart1;
...
void ESP_Init(void)
{
    ...
    // 确保 ESP32 不处于硬件复位状态（RST 低有效，必须拉高）
    ...
    // 场景：MCU 只是软复位，而 ESP32 还在透传模式且连接正常，此时无需断电重连，直接发包即可
...
static void ESP_Console_HandleLine(char *line)
{
    ...
    if (strcmp(line, "auto") == 0 || strcmp(line, "AUTO") == 0)
    {
        if (ESP_UI_SendCmd(ESP_UI_CMD_AUTO_CONNECT)) {
            ESP_Log("[ESP32SPI] auto-connect command queued.\r\n");
        } else {
            ESP_Log("[ESP32SPI] auto-connect command queue failed.\r\n");
        }
        return;
    }
        ...
        line[4] >= '0' && line[4] <= '9')
    {
        ...
        {
            frames = (frames * 10U) + (uint32_t)(line[pos] - '0');
            if (frames > 1000U)
            {
                frames = 1000U;
                break;
        ...
        if (line[pos] == 0)
        {
            ESP_SPI_FullArmFrames((uint16_t)frames);
            return;
        }
    }
...
void ESP_Console_Poll(void)
{
    ...
    /*
     * ESP32 的服务器命令是异步 SPI EVENT，不像旧 ESP UART 回包会自然流入 DMA。
...
static void ESP_SoftReconnect(void)
{
    ...
    {
			ESP_Log("[ESP] 软重连失败:无法退出透传 -> 硬复位ESP32\r\n");
    ...
    if (!ESP_Send_Cmd(cmd_buf, "CONNECT", 10000))
    {
			ESP_Log("[ESP] 软重连失败:CIPSTART -> 硬复位ESP32\r\n");
        g_link_reconnecting = 0;
        ESP_HardReset();
        ESP_Init();
        return;
...
// 硬件复位 ESP32：拉低 RST 一段时间再拉高（不需要断电）
static void ESP_HardReset(void)
{
#ifdef ESP8266_RST_Pin
